...
def send_secret():
    ...
    with open('first_secret.txt', 'r') as f:
        first_secret = int(f.read())
    f.close()
    ...
    with open('Yb.txt','r') as f:
        chteniye = f.read()
    first_shared_key = mod_pow(int(chteniye), first_secret, n)
    root_powers_text_2.insert(0, first_shared_key)
...
def diff_hell():
    ...
    with open('first_secret.txt', 'w') as f:
        f.write(str(first_secret))
    f.close()
...
label_key = tk.Label(root2, text="Xa:", font=("Matura MT Script Capitals", 12))
