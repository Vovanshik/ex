...
def Otkr_K(N,p,q):
    ...
    #     if K > len(V3) or K == 1:
    #         print(f"Ошибка! K>длины вектора V ({K}>{len(V3)}")
    #     else:
    #         break
    K = random.randint(2, len(New_V3))
    for i in range(K):
        R = random.choice(New_V3)
        New_V3.remove(R)
        V4.append(R)
    return V4
...
def close_key_S(Otkr_Key, n):
    V_minux_one_mass =[]
    for i in Otkr_Key:
        V_minus_one = st_phi(i, n)
        V_minux_one_mass.append(V_minus_one)

    Closed_Key_mass = []
    sch = 0


    for i in V_minux_one_mass:
        for j in range(n):
            sch +=1
            if (j * j) % n == i:
                Closed_Key_mass.append(j)
                break
            elif sch == n:
                Closed_Key_mass.append(-1)
        sch = 0


    return Closed_Key_mass
...
def global_opeation():
    ...
    b = [random.randint(0, 1) for _ in range(len(Otkr_Kluch))]


    y = 1
    result = False
    #Работает пользователь A
    for St in range(len(Zakr_Kluck)):
        y *= Zakr_Kluck[St] ** b[St]
    y = (y*r) % N

    #Отправляем пользователю B число y
    x1 = 1
    for St in range(len(Otkr_Kluch)):
        x1 *= Otkr_Kluch[St] ** b[St]
    x1 = (x1*(y**2)) % N
    ...
    print(f"b={b}: \nАккредитация пройдена." if result else f"b={b}: \nАккредитация не пройдена.")
    return result
